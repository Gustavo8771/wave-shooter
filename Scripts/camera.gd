...
func _process(delta: float) -> void:
	...
	if iniciar_tremor_camera:
		global_position += Vector2(rand_range(-intensidade_tremor_camera,
		 intensidade_tremor_camera), rand_range(-intensidade_tremor_camera,
		 intensidade_tremor_camera)) * delta

func tremer_tela(intensidade, tempo) -> void:
